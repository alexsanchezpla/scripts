...
#' Para realizar las curvas ROC, en primer lugar, se modeliza - para el conjunto de datos “training set”- 
#' la variable respuesta 'grupo' mediante regresión logística teniendo en cuenta la variable explicativa. 
#' Una vez ajustado el modelo se realiza las curva ROC (representación gráfica de la sensibilidad en frente a la especificidad).
...
doROC <- function(frml, titol,validation = FALSE, test, test_y,col.thres = "blue", 
                  col.ic = "#aaddddAA", x.axes = FALSE, cexMain=1)
{
  ...
  rocobj <- plot.roc(mod$y, pred,  main = titol, ci = TRUE, 
                     percent = TRUE, print.thres = "best",legacy.axes = x.axes, cex.main=cexMain) 
