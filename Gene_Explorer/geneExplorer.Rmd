...
library(httr)
set_config(
  use_proxy(url="conf_www.ir.vhebron.net", port=8080)
)
```
...
# install_github(paste('alexsanchezpla/scripts/Rpackages/', "boxplotWithDots", sep="")) # 
if(!(require(printr))) {
  install.packages(
    'printr',
    type = 'source',
...
We may plot the expression of the selected gene for each group. This can be done using boxplots, dotplots or both plots overimposed.
