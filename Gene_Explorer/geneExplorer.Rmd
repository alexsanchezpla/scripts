...
source(file.path(codeDir, "doROC.R"))
if(!(require(printr))) {
  install.packages(
    'printr',
    type = 'source',
...
For simplicity we have downloaded and stored in our `data` directory the three input files mentioned above.
...
## Exploring a single gene (2): Visualizations
...
if (length(geneIndex) > 0)
  probesetName <- rownames(topTab)[geneIndex]
whichExpres<- as.numeric(myExpres[probesetName,])
...
### Comparing expressions across group levels

Another parameter that has to be set is the covariate that is used to define the groups. This may be "Groups" or "Apocrine.grade"

```{r}
lev <- targets$Group
groupsName <- "Group"
groupLabels <- unique(targets$Group)
desc <- paste ("grouped by: ", groupsName, sep="")

lev2 <- targets$Apocrine.grade
groupsName2 <- "Apocrine.grade"
groupLabels2 <- unique(targets$Apocrine.grade)
desc2 <- paste ("grouped by: ", groupsName2, sep="")
```
...
beeswarm(whichExpres~lev2, ylab="Expression", xlab="Groups", pch=13,
         main=paste(geneName, desc2, sep="\n"), labels=groupNames2) 
beeswarm(whichExpres~lev2, ylab="Expression", xlab="Groups", pch=13,
         main=paste(geneName, desc2, sep="\n"), labels=groupNames2) 
boxplot(whichExpres~lev2, add = T, names = c("","","",""), col="#0000ff22")
par(opt)
```
...
Another common plot are ROC curves which give us an idea of \emph{how good} is the expression of a gene to distinguish between two groups. Function `doROC` encapsulates the process of building and drawing a basic ROC curve using the `pROC` package. more information is available in the package help.

```{r}
resROC <- doROC (frml = lev ~ whichExpres, titol = desc, cexMain=0.9)
```
