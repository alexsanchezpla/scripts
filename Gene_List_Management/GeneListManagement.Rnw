...
x1<- AvsB <- read.table(file.path(dataDir, "ExpressAndTop_AvsB.csv2"), head=T, sep=";", dec=",", row.names=1)
x2<- AvsL <- read.table(file.path(dataDir, "ExpressAndTop_AvsL.csv2"), head=T, sep=";", dec=",", row.names=1)
x3<- BvsL <- read.table(file.path(dataDir, "ExpressAndTop_BvsL.csv2"), head=T, sep=";", dec=",", row.names=1)
