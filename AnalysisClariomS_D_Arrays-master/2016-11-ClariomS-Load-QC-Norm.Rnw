...
Researches are interested in effects of brain stroke over the expression of genes, miRNA and proteins, and the interaction among them. In particular, this study will focus on the effects of ischemia in brain, used to mimic the effects of stroke, in gene expression using microarray technology.
\\
...
The \textbf{main objective} of this study is to find differentially expressed genes between two regions of the mouse's brain (ipsilateral and contralateral) in ischemic animals and sham animals.
...
my.targets <-read.table(file.path(dataDir,"targets.ASB3295.txt"), 
...
As shown in figure \ref{fig:boxplot}, the intensity distribution of the arrays is quite similar except for four samples: \texttt{2BT.SHAM.IP.3}, \texttt{2BT.SHAM.CL.3}, \texttt{6BT.ISC.IP.3}, \texttt{6BT.ISC.CL.3}.\\
\\
